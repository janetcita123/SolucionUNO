...
(function ($) {
    ...
    var CAPTION_TEMPLATE = '<div class="form-control file-caption {class}">\n' +
        '   <span class="glyphicon glyphicon-file"></span> <span id="archivo" class="file-caption-name"></span>\n' +
        '</div>';
    ...
    FileInput.prototype = {
                    ...
                    if (preview.length > 0 && (fileType == "any" ? (isImg || isTxt) : (fileType == "text" ? isTxt : isImg)) && typeof FileReader !== "undefined") {
                        ...
                        reader.onload = function (theFile) {
                            ...
                            if (isTxt) {
                                ...
                                content = '<div class="file-preview-frame"><div class="file-preview-text" title="' + nameVal + '">' + strText + '</div></div>' + modal;
                            }
                            else {
                                content = '<div class="file-preview-frame"><img src="' + theFile.target.result + '" class="file-preview-image" title="' + nameVal + '" alt="' + caption + '"></div>';
                            }
                            preview.append("\n" + content);
                            if (i >= total - 1) {
                        ...
                        preview.append("\n" + '<div class="file-preview-frame"><div class="file-preview-other" id="url" title ="' + nameVal + '"><h2><i class="glyphicon glyphicon-file"></i></h2>' + caption + '</div></div>');
                    }
                })(tfiles[i]);
            }
            var log = numFiles > 1 ? msgSelected.replace('{n}', numFiles) : label;
         
            self.$caption[0].innerText =nameVal;
        ...
        renderUpload: function () {
            ...
            if (isEmpty(self.uploadUrl)) {
                ...
                //content = '<button id="image1" type="file" class="' + self.uploadClass + '" value="image1" name="image1" required ></button>';
                //content = '<button type="file" id="subeActa" onclick="return false;" class="' + self.uploadClass + '"' + status + '>' + self.uploadIcon + self.uploadLabel + '</button>';
            }
            else {
